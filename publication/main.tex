...
引数 & 型 & 説明 & デフォルト \\
\hline
\texttt{--p-flickr-APIKEY} & 文字列 & Flickr APIキー（自動読込可） &  \\
\texttt{--p-flickr-Extras} & 文字列 & 取得するメタデータ（extras） & \texttt{date\_upload,date\_taken,owner\_name,geo,url\_sq,tags} \\
\texttt{--p-flickr-DateMode} & 選択 & 時間軸（upload/taken） & upload \\
\texttt{--p-flickr-DateMin} & 文字列/数値 & 期間下限（YYYY-MM-DD or UNIX秒） & 2004-01-01相当 \\
\texttt{--p-flickr-DateMax} & 文字列/数値 & 期間上限（YYYY-MM-DD or UNIX秒） & 現在時刻付近 \\
\texttt{--p-flickr-Haste} & 真偽 & 時間窓の分割並列探索 & true \\
\texttt{--p-flickr-GimmeGimme} & 文字列 & 画像ダウンロード先（任意） &  \\
\texttt{--p-flickr-ThrottleMaxConcurrent} & 数値 & API同時実行数 & 2 \\
\texttt{--p-flickr-ThrottleMinTimeMs} & 数値 & 最小待機時間（ms） & 500 \\
\hline
\end{tabular}
\end{table}
...
引数 & 型 & 説明 & デフォルト \\
\hline
\texttt{--p-gmap-APIKEY} & 文字列 & Google Places APIキー（自動読込可） &  \\
\texttt{--p-gmap-Language} & 文字列 & Places APIレスポンスの言語コード & ja \\
\texttt{--p-gmap-MaxPages} & 数値 & Text Searchで辿る最大ページ数（1〜3） & 3 \\
\texttt{--p-gmap-ThrottleMaxConcurrent} & 数値 & API同時実行数 & 2 \\
\texttt{--p-gmap-ThrottleMinTimeMs} & 数値 & 最小待機時間（ms） & 500 \\
\hline
\end{tabular}
\end{table}
...
引数 & 型 & 説明 & デフォルト \\
\hline
\texttt{--p-overpass-Endpoint} & 文字列 & Overpass interpreter endpoint URL & \url{https://overpass-api.de/api/interpreter} \\
\texttt{--p-overpass-TimeoutSeconds} & 数値 & リクエストタイムアウト（秒） & 25 \\
\texttt{--p-overpass-MaxRetries} & 数値 & HTTP/ネットワークエラー時の再試行回数 & 3 \\
\texttt{--p-overpass-UserAgent} & 文字列 & Overpassへ送るUser-Agent文字列 & Splatone-Overpass (+\url{https://github.com/YokoyamaLab/Splatone}) \\
\texttt{--p-overpass-ThrottleMaxConcurrent} & 数値 & API同時実行数（1以上） & 1 \\
\texttt{--p-overpass-ThrottleMinTimeMs} & 数値 & 最小待機時間（ms） & 1500 \\
\hline
\end{tabular}
\end{table}
