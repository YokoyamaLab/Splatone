...
Providerはデータを集める対象となるサービス毎に構成し，現在のバージョンではFlickr, Google Places Text Search（gmap）, OpenStreetMap Overpassの3種類を同梱しているが，アーキテクチャはオープンである．
...
本ツールのCLIでは，カテゴリ集合を1つの文字列として与える（例: \texttt{-k} / \texttt{--keywords}）．基本的な表記は以下の通りである．
\begin{itemize}
  \item カテゴリ区切り: \texttt{|}
  \item ラベル付け: \texttt{ラベル=検索指定}
  \item 明示色指定（任意）: \texttt{ラベル\#RRGGBB=検索指定}（例: \texttt{カフェ\#ff5f5f=...}）
\end{itemize}
ここで「検索指定」の内部表現（例えば \texttt{,} をORとして解釈するか，複数クエリとして分割するか）はProviderの種類に依存する（後述）．
...
現在のバージョンではFlickr, Google Places Text Search（gmap）, OpenStreetMap Overpassの3種類のProviderを提供しており，アーキテクチャはオープンで第三者が新たなProviderを追加する事も可能である．

Providerごとの検索指定（\texttt{--keywords}の右辺）の解釈は次の通りである．
\begin{itemize}
  \item Flickr（\texttt{flickr}）: タグキーワード（複数語）によりジオタグ付き写真を検索する．
  \item Google Places Text Search（\texttt{gmap}）: Text Search のクエリ文字列（自然言語・空白を含む文字列）を送信しPOIを取得する．同一カテゴリ内で\texttt{,}区切りにすると複数クエリとして実行し合算する．
  \item OpenStreetMap Overpass（\texttt{overpass}）: \texttt{amenity=cafe} 等のタグ条件（key/value/正規表現等）でノード・ウェイ・リレーションを取得する．\texttt{node:}等のプレフィックスで幾何種を限定できる．同一カテゴリ内で\texttt{,}区切りにすると複数条件として実行し合算する．
\end{itemize}
...
Google Places Text Search（\texttt{gmap}）を利用する場合も同様に \texttt{-p gmap} を指定する．APIキーは \texttt{--p-gmap-APIKEY} で渡すほか，環境変数 \texttt{API\_KEY\_gmap} またはファイル \texttt{.API\_KEY.gmap} から読み込ませることもできる．

\begin{commandline}
npx -y -p splatone@latest crawler -p gmap -k "カフェ=cafe tokyo|観光=landmark tokyo" \
--vis-bulky --city "Tokyo" --p-gmap-APIKEY="AIzaxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
\end{commandline}

OpenStreetMap Overpass（\texttt{overpass}）ではタグ条件でPOIを収集する．以下はカフェ（amenity）と文化施設（tourism）を比較する例である．

\begin{commandline}
npx -y -p splatone@latest crawler -p overpass -k "カフェ\#ff5f5f=amenity=cafe,amenity=coffee_shop|文化施設\#3366ff=tourism=museum,tourism=gallery" \
--vis-bulky --city "Kyoto"
\end{commandline}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{svg-inkscape/overall_svg-raw.pdf}
...
\section{Provider for SNS Data Crawling}
\label{sec:provider}

本章では，Splatone Crawlerが複数のジオソーシャルデータサービスを同一の枠組みで扱うためのProvider機構と，領域分割に基づくクローリング実行モデルを説明する．Providerは外部サービス固有のAPI呼び出しやレスポンス形式の差異を吸収し，GeoJSONを基底とするSUFS/Nakedへ正規化した点群を出力する．また，クローリングはHexGrid単位で分割されたサブクエリをワーカープロセスに割り当てることで，API制限を考慮しつつ並列性を確保する．

\subsection{Providerプラグインの設計}
Providerは，\texttt{providers/<id>/} 配下に配置されるプラグインであり，\texttt{index.js} にクラスをエクスポートすることで読み込まれる．Providerクラスは基底クラス\texttt{ProviderBase}を継承し，少なくとも以下の責務を担う．
\begin{itemize}
  \item CLIオプション（APIキー等）の定義（\texttt{yargv()}）
  \item オプション検証・補完（\texttt{check()}：APIキー自動読込など）
  \item 与えられた領域分割（HexGrid）とカテゴリ定義に基づくクローリング開始（\texttt{crawl()}）
\end{itemize}

ProviderのCLI引数は一貫して \texttt{--p-<providerId>-<OptionName>} のプレフィックスで与えられる．例えばFlickrでは \texttt{--p-flickr-APIKEY} のように指定する．この命名規約により，複数Providerのオプションが混在しても衝突せず，Provider追加時も既存CLIとの互換性を保ちやすい．

Providerのロードは動的である．起動時に\texttt{providers}ディレクトリを走査し，\texttt{providerLoader}が依存関係（\texttt{dependencies}）を考慮してインスタンス化する．これにより，特定のProviderを追加・差し替えしてもメインルーチン側の改変を最小化できる．

\subsection{領域分割とサブクエリ生成}
Splatone Crawlerでは，ユーザがWeb UI上で指定する探索領域（矩形またはポリゴン）に対し，セルサイズをパラメータとしてHexGridを生成する（第\ref{sec:overview}章）．ProviderはこのHexGridの各セル（以降，Hexセル）を独立した問い合わせ単位として扱う．

さらにカテゴリ（複数キーワード集合）が定義されているため，実際に発行されるサブクエリ数は，概ね
$$
N_{task} = |H| \times |C|
$$
となる（$|H|$ はHexセル数，$|C|$ はカテゴリ数）．各サブクエリは，\textbf{(Hexセル，カテゴリ)} の組で識別され，クローリング進捗もこの粒度で管理される．

実装上は，Providerの\texttt{crawl()}が各Hexセル・各カテゴリについてワーカ実行要求（\texttt{splatone:start}イベント）を発火する．イベントのペイロードには，Hexセルポリゴン，その外接BBox，補助的な三角分割（後述），カテゴリ名，検索語（タグ集合またはクエリ文字列），およびProvider固有オプションが含まれる．

\subsection{ワーカープロセスによる非同期クローリング}
外部APIは一般にレート制限を持つため，単純に全サブクエリを同時発行すると容易に制限超過やタイムアウトを引き起こす．本ツールでは，サブクエリ実行をワーカープロセスに分離し，かつProviderごとにスロットリングを行うことで，安定的な収集を行う．

各Providerには対応する\texttt{worker.js}が存在し，実際のAPI呼び出しとレスポンスのGeoJSON化を担当する．メインプロセスはワーカから返る結果を逐次統合し，重複ID（例：Flickrの写真ID，Google Placesの\texttt{place\_id}）の除去，進捗の更新，およびWeb UIへの通知を行う．

\subsection{継続クローリングと\texttt{nextProviderOptions}}
外部APIのページングや，時間窓の分割などにより，単一サブクエリが複数回のリクエストに分割される場合がある．Splatone Crawlerはこの状況に対応するため，ワーカが \texttt{nextProviderOptions} を返す設計を採用している．

ワーカは，得られた結果（\texttt{photos}：FeatureCollection）に加えて，次回問い合わせに必要なトークン（例：ページトークン，更新した時間範囲）を埋め込んだ\texttt{providerOptions}の差分配列を返す．メインプロセスは \texttt{final=false} の場合に，この差分を用いて同一の (Hexセル，カテゴリ) に対する次のワーカタスクを自動的に再スケジュールする．この再帰的スケジューリングにより，Provider固有のページング／分割戦略を，メインルーチンを汚さずに実装できる．

\subsection{SUFS/Nakedへの正規化}
Providerワーカの出力は，GeoJSON FeatureCollectionであり，点ジオメトリを持つFeatureの\texttt{properties}に，最低限以下のメタデータを付与する．
\begin{itemize}
  \item \texttt{splatone\_provider}: 取得元Provider識別子
  \item \texttt{splatone\_hexId}: 属するHexセルID
  \item \texttt{splatone\_triId}: Hexセル内の補助三角形ID（任意）
  \item \texttt{tooltipContent}: 地図上のポップアップ表示用HTML（任意）
\end{itemize}
これにより，可視化側はProvider種別に依存せず，カテゴリ色や集約単位（Hex/三角形）に基づく処理を統一的に実施できる．

\subsection{同梱Provider}
本節では，現バージョンで同梱している3種類のProvider（Flickr，Google Places Text Search，Overpass）について，クエリ方法と継続クローリング戦略を述べる．

\subsubsection{Flickr Provider}
Flickr ProviderはFlickr API \texttt{flickr.photos.search}を用い，指定領域（BBox）内でジオタグを持つ写真を検索し，撮影位置（緯度経度）とメタデータをSUFS/NakedのFeatureとして返す．1回の問い合わせでは最大250件を取得し，APIレスポンスに含まれるページ数・総件数に応じて継続クローリングを行う．

Flickrではページングに加え，時間窓（投稿日時または撮影日時）の指定が可能である．本Providerでは\texttt{DateMin}と\texttt{DateMax}で時間範囲を制約し，新しい投稿から順に探索する．取得件数が多くページ数が閾値を超える場合は，時間範囲を二分割して並列探索する（\texttt{Haste}）．このときワーカは2本の\texttt{nextProviderOptions}を返し，時間窓を分割した2つのタスクとして再投入される．

また，単一投稿者による高頻度投稿などにより短時間に大量の結果が返る場合，時間窓が極端に狭くなって探索が停滞しうる．本実装では投稿者分布と時間窓幅を観測し，一定条件下で時間をスキップすることで探索の安定性を高めている．

Flickr Providerの主要なオプションを表\ref{tab:flickr_provider_params}に示す．APIキーは明示指定に加え，ローカルファイル（\texttt{.API\_KEY.flickr}）または環境変数からの自動読込にも対応する．

\begin{table}[h]
\centering\small
\setlength{\tabcolsep}{3pt}
\caption{Flickr Providerの主なコマンドライン引数}
\label{tab:flickr_provider_params}
\begin{tabularx}{\textwidth}{L{0.40\textwidth} L{0.09\textwidth} Y L{0.16\textwidth}}
\hline
引数 & 型 & 説明 & デフォルト \\
\hline
\url{--p-flickr-APIKEY} & 文字列 & Flickr APIキー（自動読込可） &  \\
\url{--p-flickr-Extras} & 文字列 & 取得するメタデータ（extras） & \url{date_upload,date_taken,owner_name,geo,url_sq,tags} \\
\url{--p-flickr-DateMode} & 選択 & 時間軸（upload/taken） & upload \\
\url{--p-flickr-DateMin} & 文字列/数値 & 期間下限（YYYY-MM-DD or UNIX秒） & 2004-01-01相当 \\
\url{--p-flickr-DateMax} & 文字列/数値 & 期間上限（YYYY-MM-DD or UNIX秒） & 現在時刻付近 \\
\url{--p-flickr-Haste} & 真偽 & 時間窓の分割並列探索 & true \\
\url{--p-flickr-GimmeGimme} & 文字列 & 画像ダウンロード先（任意） &  \\
\url{--p-flickr-ThrottleMaxConcurrent} & 数値 & API同時実行数 & 2 \\
\url{--p-flickr-ThrottleMinTimeMs} & 数値 & 最小待機時間（ms） & 500 \\
\hline
\end{tabularx}
\end{table}

\subsubsection{Google Places Text Search Provider (gmap)}
gmap ProviderはGoogle Places Text Search APIを用い，Hexセルの重心位置と半径（セルサイズから導出）を用いて地点情報を検索し，\texttt{place\_id} をIDとしてFeatureを生成する．Text Searchは \texttt{next\_page\_token} によるページングを持つため，ワーカはトークンを\texttt{nextProviderOptions}に格納し，最大ページ数（\texttt{MaxPages}）まで継続取得する．またトークンは即時に有効化されないため，次ページ取得前に待機時間（約2.2秒）を挿入する．

検索語はカテゴリ定義から与えられるが，カンマ区切りで複数のクエリ語を指定した場合，それぞれを独立なバリアントとして実行し，(Hexセル，カテゴリ) 内に複数タームとして統合する．これにより，類義語・表記揺れを簡易にカバーできる．

gmap Providerの主要なオプションを表\ref{tab:gmap_provider_params}に示す．APIキーは明示指定に加え，ローカルファイル（\texttt{.API\_KEY.gmap}）または環境変数からの自動読込にも対応する．

\begin{table}[h]
\centering\small
\setlength{\tabcolsep}{3pt}
\caption{gmap Providerの主なコマンドライン引数}
\label{tab:gmap_provider_params}
\begin{tabularx}{\textwidth}{L{0.40\textwidth} L{0.09\textwidth} Y L{0.16\textwidth}}
\hline
引数 & 型 & 説明 & デフォルト \\
\hline
\url{--p-gmap-APIKEY} & 文字列 & Google Places APIキー（自動読込可） &  \\
\url{--p-gmap-Language} & 文字列 & Places APIレスポンスの言語コード & ja \\
\url{--p-gmap-MaxPages} & 数値 & Text Searchで辿る最大ページ数（1〜3） & 3 \\
\url{--p-gmap-ThrottleMaxConcurrent} & 数値 & API同時実行数 & 2 \\
\url{--p-gmap-ThrottleMinTimeMs} & 数値 & 最小待機時間（ms） & 500 \\
\hline
\end{tabularx}
\end{table}

\subsubsection{Overpass Provider}
Overpass ProviderはOverpass APIを用い，OpenStreetMap上のPOI（node/way/relation）を収集する．クエリは，タグキーのみ（例：\texttt{amenity}）や，キーと値の等価・正規表現など（例：\texttt{amenity=cafe}，\texttt{name=~Tokyo}）を指定でき，必要に応じて対象型（\texttt{node:...} など）も制約できる．ワーカは指定BBoxを付与したOverpass QLを生成し，HTTP POSTで問い合わせ，返却された要素から代表座標（nodeの座標またはway/relationのcenter等）をPoint Featureに正規化する．

Overpass APIは過負荷時に一時的なエラーを返すことがあるため，タイムアウトと再試行回数を指定できる．またUser-Agentを明示し，適切なアクセスを行うようにしている．

Overpass Providerの主要なオプションを表\ref{tab:overpass_provider_params}に示す．

\begin{table}[h]
\centering\small
\setlength{\tabcolsep}{3pt}
\caption{Overpass Providerの主なコマンドライン引数}
\label{tab:overpass_provider_params}
\begin{tabularx}{\textwidth}{L{0.40\textwidth} L{0.09\textwidth} Y L{0.16\textwidth}}
\hline
引数 & 型 & 説明 & デフォルト \\
\hline
\url{--p-overpass-Endpoint} & 文字列 & Overpass interpreter endpoint URL & \url{https://overpass-api.de/api/interpreter} \\
\url{--p-overpass-TimeoutSeconds} & 数値 & リクエストタイムアウト（秒） & 25 \\
\url{--p-overpass-MaxRetries} & 数値 & HTTP/ネットワークエラー時の再試行回数 & 3 \\
\url{--p-overpass-UserAgent} & 文字列 & Overpassへ送るUser-Agent文字列 & Splatone-Overpass (+\url{https://github.com/YokoyamaLab/Splatone}) \\
\url{--p-overpass-ThrottleMaxConcurrent} & 数値 & API同時実行数（1以上） & 1 \\
\url{--p-overpass-ThrottleMinTimeMs} & 数値 & 最小待機時間（ms） & 1500 \\
\hline
\end{tabularx}
\end{table}
...
引数 & 型 & 説明 & デフォルト \\
\hline
  \url{--vis-bulky} & 真偽 & Bulky Visualizationを有効にする & false \\
  \url{--v-bulky-Radius} & 数値 & Point Markerの半径 & 5 \\
  \url{--v-bulky-Stroke} & 真偽 & Point Markerの線の有無 & true \\
  \url{--v-bulky-Weight} & 数値 & Point Markerの線の太さ & 1 \\
  \url{--v-bulky-Opacity} & 数値 & Point Markerの線の透明度 & 1 \\
  \url{--v-bulky-Filling} & 真偽 & Point Markerの塗りの有無 & true \\
  \url{--v-bulky-FillOpacity} & 数値 & Point Markerの塗りの透明度 & 0.5 \\
\hline
\end{tabularx}
\end{table} 
...
引数 & 型 & 説明 & デフォルト \\ 
\hline
  \url{--vis-marker-cluster} & 真偽 & Marker-cluster Visualizationを有効にする & false \\
  \url{--v-marker-cluster-}\allowbreak\url{MaxClusterRadius} & 数値 & クラスタを構成する範囲(半径) & 80 \\
\hline
\end{tabularx}
\end{table} 
...
引数 & 型 & 説明 & デフォルト \\
\hline
  \url{--vis-voronoi} & 真偽 & Voronoi Visualizationを有効にする & false \\
  \url{--v-voronoi-MaxSitesPerHex} & 数値 & 各Hex内で残す最大サイト数（0=無制限） & 0 \\
  \url{--v-voronoi-MinSiteSpacingMeters} & 数値 & サイト間の最小距離[メートル] & 50 \\
\hline
\end{tabularx}
\end{table}
...
引数 & 型 & 説明 & デフォルト \\
\hline
  \url{--vis-pie-charts} & 真偽 & Pie-charts Visualizationを有効にする & false \\
  \url{--v-pie-charts-MaxRadiusScale} & 数値 & Hex内接円半径に対する最大半径スケール（0–1.5） & 0.9 \\
  \url{--v-pie-charts-MinRadiusScale} & 数値 & 最大半径に対する最小半径スケール（0–1） & 0.25 \\
  \url{--v-pie-charts-StrokeWidth} & 数値 & Pie Chart輪郭線の太さ(px) & 1 \\
  \url{--v-pie-charts-BackgroundOpacity} & 数値 & 最大半径ガイドリングの透明度（0–1） & 0.2 \\
\hline
\end{tabularx}
\end{table}
...
引数 & 型 & 説明 & デフォルト \\
\hline
  \url{--vis-majority-hex} & 真偽 & Majority-hex Visualizationを有効にする & false \\
  \url{--v-majority-hex-Hexapartite} & 真偽 & Hex内カテゴリ頻度に応じて六角形を分割色彩 & false \\
  \url{--v-majority-hex-HexOpacity} & 数値 & 六角形の線の透明度 & 1 \\
  \url{--v-majority-hex-HexWeight} & 数値 & 六角形の線の太さ & 1 \\
  \url{--v-majority-hex-MaxOpacity} & 数値 & 正規化後の最大塗り透明度 & 0.9 \\
  \url{--v-majority-hex-MinOpacity} & 数値 & 正規化後の最小塗り透明度 & 0.5 \\
\hline
\end{tabularx}
\end{table}
...
引数 & 型 & 説明 & デフォルト \\
\hline
  \url{--vis-heat} & 真偽 & Heatmap Visualizationを有効にする & false \\
  \url{--v-heat-Radius} & 数値 & ヒートマップブラーの半径（Unitsで指定した距離単位） & 50 \\
  \url{--v-heat-Units} & 文字列 & Radiusに使用する距離単位（kilometers/meters/miles） & meters \\
  \url{--v-heat-MinOpacity} & 数値 & ヒートマップの最小透明度 & 0 \\
  \url{--v-heat-MaxOpacity} & 数値 & ヒートマップの最大透明度 & 1 \\
  \url{--v-heat-MaxValue} & 数値 & ヒートマップ強度の最大値（未指定時は自動推定） & （未指定） \\
  \url{--v-heat-WeightThreshold} & 数値 & 近傍点数がこの値未満の点は描画しない & 1 \\
\hline
\end{tabularx}
\end{table}
...
引数 & 型 & 説明 & デフォルト \\
\hline
  \url{--vis-dbscan} & 真偽 & DBSCAN Visualizationを有効にする & false \\
  \url{--v-dbscan-Eps} & 数値 & DBSCANのeps（クラスタ判定距離） & 0.6 \\
  \url{--v-dbscan-MinPts} & 数値 & クラスタ確定に必要な点数 & 6 \\
  \url{--v-dbscan-Units} & 文字列 & epsの距離単位（kilometers/meters/miles） & kilometers \\
  \url{--v-dbscan-StrokeWidth} & 数値 & ポリゴン輪郭の太さ & 2 \\
  \url{--v-dbscan-StrokeOpacity} & 数値 & ポリゴン輪郭の透明度 & 0.9 \\
  \url{--v-dbscan-FillOpacity} & 数値 & ポリゴン塗りの透明度 & 0.35 \\
  \url{--v-dbscan-DashArray} & 文字列 & LeafletのdashArray指定（例: "4 6"） & （空文字） \\
  \url{--v-dbscan-KernelScale} & 数値 & KDEカーネル半径をepsの何倍にするか & 1 \\
  \url{--v-dbscan-GridSize} & 数値 & KDE計算用グリッド解像度（長辺方向セル数） & 80 \\
  \url{--v-dbscan-ContourPercent} & 数値 & 最大密度に対する等値線レベル（0-1） & 0.4 \\
\hline
\end{tabularx}
\end{table}
...
Splatone Crawlerはデータ収集部をProviderとしてモジュール化し，コアのクロール制御（Hex分割，進捗管理，結果バンドル化）と，各サービス固有のAPI呼び出し・正規化処理を分離している．
Providerは基底クラス \texttt{ProviderBase} を継承し，少なくとも \texttt{static version} を持つクラスとして実装する．ロード時には \texttt{lib/providerLoader.js} がProviderディレクトリ（既定では \texttt{providers/}）直下の各サブディレクトリから \texttt{index.js} を動的importし，メタ情報（ID，バージョン，依存関係）を取得してから，依存関係（\texttt{static dependencies}）に基づくトポロジカルソートで初期化順序を決定する．これにより，認証や共通ユーティリティ等のProvider間依存を明示的に扱える．

第三者によるProvider追加は，(1) リポジトリ内の \texttt{providers/<id>/} に実装を配置する方法，または (2) NPMパッケージとして配布し \texttt{--plugin} オプションで明示ロードする方法の2通りを提供する．後者の場合，\texttt{splatone-provider-<id>} の命名規約を推奨し，パッケージのdefault exportとしてProviderクラスを公開する．

本システムではProvider処理をワーカスレッドで実行するため，NPM配布Providerはワーカエントリも同梱する必要がある．具体的には \texttt{"<pkg>/worker"}（例：\texttt{exports} に \texttt{"./worker": "./worker.js"} を追加）として解決可能な形で提供する．これにより，ホスト側はProvider本体とワーカ実装を対応づけ，Piscinaによる並列実行へ安全に組み込める．
...
可視化部も同様にVisualizerとしてモジュール化し，サーバ側の前処理とブラウザ側の描画処理を分離して追加できる．リポジトリ同梱Visualizerは \texttt{visualizer/<id>/node.js}（サーバ側クラス）と \texttt{visualizer/<id>/web.js}（ブラウザ側スクリプト）を基本とし，必要に応じて \texttt{visualizer/<id>/public/} 以下に補助的な静的アセットを配置する．

第三者Visualizerの配布もNPMパッケージとして行える．\texttt{--plugin} で \texttt{splatone-visualizer-<id>} を明示ロードすると，ホストはdefault exportからVisualizerクラスを取得し，ブラウザ側エントリ（\texttt{"<pkg>/web"} またはパッケージ直下の \texttt{web.js}）を解決して配信対象に登録する．

サードパーティープラグインの安全性と安定性の観点から，サーバはVisualizerディレクトリ内の \texttt{node.js} への直接アクセスを遮断し，必要な \texttt{web.js} と \texttt{/public} 配下のみをホワイトリスト方式で配信する．これにより，プラグインの内部実装（サーバ側クラス）を不用意に外部公開せずに，ブラウザ側に必要な最小限の資産だけを提供できる．

\begin{commandline}
npx -y -p splatone@latest crawler \
  --plugin splatone-visualizer-foo \
  --vis-foo -p flickr -k "..."
\end{commandline}

また，Visualizerはオプションスキーマを宣言することで，CLI引数（\texttt{--v-<id>-*}）を自動的に追加できる．これにより第三者Visualizerであっても，コアを変更せずにパラメータ調整UI（CLIおよびブラウザ）と統合できる．

以上の設計により，データソース（Provider）と表現（Visualizer）を独立に追加・交換でき，特定ドメイン（例：イベント，観光，交通）に特化した収集ロジックや可視化表現をコミュニティベースで拡張できる．一方で，プラグインはホストプロセスと同等の実行権限で動作するため，利用者は信頼できる配布元のプラグインを選択し，実験の再現性確保のために利用したプラグイン名とバージョン（およびCLI引数）を記録する必要がある．
