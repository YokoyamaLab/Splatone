...
export async function loadProviders({
  ...
  packages = [],                 // ['splatone-provider-foo', ...]
  ...
  for (const spec of pkgList) {
    ...
    try {
      ...
      throw new Error(`[provider] failed to import package "${spec}": ${reason}`);
    }
    const ProviderClass = mod?.default ?? mod;
    validateClass(ProviderClass, spec);
    const id = ProviderClass.id || deriveIdFromPackageName(spec);
    metas.push({
      id,
      folder: null,
      entry: spec,
  ...
  for (const m of metas) {
    if (idSeen.has(m.id)) {
      throw new Error(`[provider] duplicated id: ${m.id}`);
    }
    idSeen.add(m.id);
  }
  ...
  for (const id of order) {
    ...
    if (meta.package) {
      // crawler 側で worker などの関連エントリを解決できるよう、出自を保持する
      Object.defineProperty(instance, '__splatonePackage', {
        value: meta.package,
        enumerable: false,
        configurable: false,
        writable: false,
...
function deriveIdFromPackageName(spec) {
  const base = String(spec).split('/').filter(Boolean).pop() || String(spec);
  const m = base.match(/^splatone-provider-(.+)$/);
  if (m) return m[1];
  return base;
}
