...
export async function saveGeoJsonObjectAsStream(geoJsonObject, outfile) {
  const dir = path.join(PUBLIC_DIR, 'out');
...
export function buildProvidersOptions(argv, providerIds) {
  return buildOptions('p', argv, providerIds);
}
...
/* API Key読み込み */
export async function loadAPIKey(provider = 'flickr') {
  //ファイルチェック→環境変数チェック

  const filePath = ".API_KEY." + provider;
  ...
  try {
    ...
    //console.log(`[API KEY (${provider}})] Read from FILE`);
    key = raw.trim();
  } catch (err) {
    if (Object.prototype.hasOwnProperty.call(process.env, "API_KEY_" + provider)) {
      //console.log(`[API KEY (${provider}})] Read from ENV`);
      key = process.env["API_KEY_" + provider] ?? null;
    } else {
...
/*
export default {
  dfsObject, bboxSize, saveGeoJsonObjectAsStream, buildProvidersOptions, loadAPIKey
};*/
